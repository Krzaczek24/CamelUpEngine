...
namespace CamelUpEngine.Helpers
{
    ...
    {
        public static IReadOnlyCollection<IAvailableTypingCard> CardRepository { get; } = ColourHelper.AllCardColours.SelectMany(colour => Enum.GetValues<TypingCardValue>().Select(value => new AvailableTypingCard(new TypingCard(colour, value)))).ToList();
